...
namespace sell.webapi.Repository
{
    public class UserInfoRepository : IUserInfoRepository
    {
        ...
        public async Task<IEnumerable<UserInfoTable>> GetAllList()
        {
            ...
            var query = "select user_id , user_name ,user_pwd ,user_phone ,user_gender ,user_flag ,create_time ,update_time  from user_info";
            var list = await _dbContext.Select<UserInfoTable>(query);
            return list;
        }

        public UserInfoTable GetUserInfoById(int userId)
        {
            ...
            var user = _dbContext.SelectFirst<UserInfoTable>(query, new { UserId = userId });
            return user;
        }

        public async Task<IEnumerable<UserInfoTable>> GetUserInfoList(int pageIndex, int pageSize)
        {
            ...
            var list = await _dbContext.Select<UserInfoTable>(sql, new { start = (pageIndex - 1) * pageSize, end = pageSize });
            return list;
        }
